...
//sum function
//total ACTIVE working hours of an employee on particular range of dates

// const totalActiveHoursForRange = async (table, column, startDate,endDate ,user_id) => {
//     const query = `
//       SELECT ${column}, SUM(active_hours) AS active_hours_range
//       FROM ${table}
//       WHERE date between ? AND ?  AND ${column} = ?
//       GROUP BY ${column}
//     `;
  
//     const [rows] = await pool.execute(query, [startDate,endDate , user_id]);
...
//AVerage fucntions
//   //Average working hours of an employee/student
//   const getAvgWorkingHoursOnRange = async (table, user_id, startDate,column, endDate) => {
//     const query = `
//       SELECT ${column}, SEC_TO_TIME(AVG(TIME_TO_SEC(total_hours))) AS avg_working_hours
//       FROM ${table}
//       WHERE ${column} = ? AND date BETWEEN ? AND ?
//       GROUP BY ${column}
//     `;
...
//     //Average ACTIVE working hours of an employee/student
//     const getActiveAvgWorkingHoursOnRange = async (table, user_id, startDate,column, endDate) => {
//         const query = `
//           SELECT ${column}, SEC_TO_TIME(AVG(TIME_TO_SEC(active_hours))) AS avg_active_hours
//           FROM ${table}
//           WHERE ${column} = ? AND date BETWEEN ? AND ?
//           GROUP BY ${column}
//         `;
